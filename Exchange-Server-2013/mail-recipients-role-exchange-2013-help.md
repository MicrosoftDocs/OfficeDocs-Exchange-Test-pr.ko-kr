---
title: '메일 받는 사람에 게 역할: Exchange 2013 Help'
TOCTitle: 메일 받는 사람에 게 역할
ms:assetid: 7cf1f1c7-4f32-4446-ac04-99a7f9df2059
ms:mtpsurl: https://technet.microsoft.com/ko-kr/library/Dd876911(v=EXCHG.150)
ms:contentKeyID: 50483490
ms.date: 05/22/2018
...
이 역할 유형을 사용하여 메일 사용 가능 공용 폴더 또는 메일 그룹을 관리할 수는 없습니다. 이러한 개체를 관리하려면 다음 역할을 사용하십시오.
...
조직에서 받는 사람 만들기를 수행하는 그룹과 관리를 수행하는 그룹이 나누어진 사용 권한 분할 모델을 사용하는 경우에는 받는 사람 만들기를 수행하는 그룹에 `Mail Recipient Creation` 역할을 할당하고 받는 사람 관리를 수행하는 그룹에 `Mail Recipients` 역할을 할당합니다. 자세한 내용은 다음 항목을 참조하십시오.
...
받는 사람 범위 외에 이 역할에 포함된 **Connect-Mailbox** 및 **Enable-Mailbox** cmdlet도 데이터베이스 구성 범위를 사용하여 범위가 지정됩니다. 데이터베이스 구성 범위는 cmdlet으로 새 사서함을 만들 수 있는 데이터베이스를 제어합니다. 사서함을 만들 데이터베이스는 데이터베이스 범위 안에 있어야 합니다. 이 조건은 두 cmdlet 중 하나에서 *Database* 매개 변수를 사용하여 데이터베이스를 지정할 때 또는 자동 사서함 배포를 통해 데이터베이스 선택하도록 하는 경우에 적용됩니다. 자세한 내용은 [관리 역할 범위 이해 (영문)](understanding-management-role-scopes-exchange-2013-help.md) 항목을 참조하십시오.
