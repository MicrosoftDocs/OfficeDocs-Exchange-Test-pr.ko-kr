...
받는 사람 범위 외에도 이 역할에 포함된 **New-MoveRequest** cmdlet 역시 데이터베이스 구성 범위를 사용하여 범위를 설정합니다. 데이터베이스 구성 범위는 cmdlet이 사서함을 이동할 수 있는 대상 데이터베이스를 제어합니다. 사서함을 이동할 대상 데이터베이스는 데이터베이스 범위 내에 있어야 합니다. 이 조건은 **New-MoveRequest** cmdlet의 *TargetDatabase* 매개 변수를 사용하여 데이터베이스를 지정할 때 또는 자동 사서함 배포를 통해 적절한 데이터베이스를 선택할 수 있도록 하는 경우에 적용됩니다. 자세한 내용은 [관리 역할 범위 이해 (영문)](understanding-management-role-scopes-exchange-2013-help.md) 항목을 참조하십시오.
