---
title: 'Exchange 서버 역할: Exchange 2013 Help'
TOCTitle: Exchange 서버 역할
ms:assetid: 5a626746-8d23-409c-a604-7e565c429990
ms:mtpsurl: https://technet.microsoft.com/ko-kr/library/Dd876887(v=EXCHG.150)
ms:contentKeyID: 50483188
ms.date: 05/22/2018
...
`Exchange Servers` 관리 역할을 사용하면 관리자가 개별 서버에서 다음 작업을 수행할 수 있습니다.

  - 데이터베이스 가용성 그룹 추가/제거 및 데이터베이스 복사본 구성

  - 통합 메시징 서비스 설정, 해제 및 구성

  - 사서함 및 클라이언트 액세스 서버의 전송 구성 수정

  - 클라이언트 액세스 서버에서 외부에서 Microsoft Outlook 사용의 사용 여부 설정

  - 사서함 및 클라이언트 액세스 서버 구성 수정

  - 클라이언트 액세스 서버에서 외부에서 Outlook 사용 구성 수정

  - 사서함 서버의 콘텐츠 필터링 구성 수정

  - 일반적인 Exchange 서버 구성 수정
