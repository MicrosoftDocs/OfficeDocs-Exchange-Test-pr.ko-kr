---
title: 'Mail Recipient Creation 역할: Exchange 2013 Help'
TOCTitle: Mail Recipient Creation 역할
ms:assetid: 81081845-9ebf-4888-8f9f-fe7cf2704486
ms:mtpsurl: https://technet.microsoft.com/ko-kr/library/Dd876915(v=EXCHG.150)
ms:contentKeyID: 50483533
ms.date: 05/22/2018
...
이 역할을 통해 공용 폴더를 메일 사용이 가능하도록 설정할 수 없습니다. 공용 폴더를 메일 사용이 가능하도록 설정하려면 `Mail Enabled Public Folders` 역할을 사용해야 합니다. 자세한 내용은 [메일 사용 가능 공용 폴더 역할](mail-enabled-public-folders-role-exchange-2013-help.md)을 참조하십시오.

조직에서 받는 사람 만들기와 받는 사람 관리가 서로 다른 그룹에서 수행되는 RBAC(역할 기반 액세스 제어) 사용 권한 분할 모델을 유지하는 경우 받는 사람 만들기를 수행하는 관리 역할 그룹에 `Mail Recipient Creation` 역할을 할당하고 받는 사람 관리를 수행하는 역할 그룹에 `Mail Recipients` 역할을 할당합니다.

조직이 Active Directory 사용 권한 분할을 사용하도록 설정한 경우 이 관리 역할에 위임되지 않은 관리 역할이 모두 제거됩니다. Active Directory 사용 권한 분할을 사용하도록 설정하면 Active Directory 관리 도구를 사용하는 Active Directory 관리자만이 사용자 및 보안 그룹과 같은 새로운 보안 주체를 만들 수 있습니다.

RBAC 및 Active Directory 사용 권한 분할에 대한 자세한 내용은 [분할 권한 이해](understanding-split-permissions-exchange-2013-help.md)를 참조하십시오.
...
받는 사람 범위 외에 이 역할에 포함된 **New-Mailbox** cmdlet의 범위도 데이터베이스 구성 범위를 사용하여 지정됩니다. 데이터베이스 구성 범위는 cmdlet이 새 사서함을 만들 수 있는 데이터베이스를 제어합니다. 사서함을 만들 데이터베이스가 데이터베이스 범위 내에 있어야 합니다. 이 조건은 **New-Mailbox** cmdlet의 *Database* 매개 변수를 사용하여 데이터베이스를 지정하거나 자동 사서함 배포가 사용자에게 필요한 데이터베이스를 선택할 수 있도록 허용한 경우에 적용됩니다. 자세한 내용은 [관리 역할 범위 이해 (영문)](understanding-management-role-scopes-exchange-2013-help.md) 항목을 참조하십시오.
