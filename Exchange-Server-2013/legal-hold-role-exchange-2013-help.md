...
받는 사람 범위 외에 이 역할에 포함된 **Enable-Mailbox** cmdlet의 범위도 데이터베이스 구성 범위를 사용하여 지정됩니다. 데이터베이스 구성 범위는 cmdlet이 새 사서함을 만들 수 있는 데이터베이스를 제어합니다. 사서함을 만들 데이터베이스가 데이터베이스 범위 내에 있어야 합니다. 이는 **Enable-Mailbox** cmdlet의 *Database* 매개 변수를 사용하여 데이터베이스를 지정하거나 자동 사서함 배포가 사용자에게 필요한 데이터베이스를 선택할 수 있도록 허용한 경우에 적용됩니다. 자세한 내용은 [관리 역할 범위 이해 (영문)](understanding-management-role-scopes-exchange-2013-help.md)를 참조하십시오.
