...
이 역할에 포함된 **Set-MailboxDatabaseCopy** 및 **Remove-MailboxDatabaseCopy** cmdlet을 사용하려면 구성 또는 제거할 데이터베이스가 데이터베이스 범위 내에 있고 데이터베이스가 상주하는 서버는 서버 범위 내에 있어야 합니다. 범위에 대한 자세한 내용은 [관리 역할 범위 이해 (영문)](understanding-management-role-scopes-exchange-2013-help.md)를 참조하십시오.
