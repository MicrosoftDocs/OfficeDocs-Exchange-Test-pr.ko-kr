---
title: '데이터베이스 복사본 역할: Exchange 2013 Help'
TOCTitle: 데이터베이스 복사본 역할
ms:assetid: 71a95f5b-1f75-4ae9-9ee9-515c3a19d394
ms:mtpsurl: https://technet.microsoft.com/ko-kr/library/Dd876905(v=EXCHG.150)
ms:contentKeyID: 50483417
ms.date: 05/22/2018
...
관리자는 `Database Copies` 관리 역할을 사용하여 개별 서버에서 데이터베이스 복사본을 추가, 제거, 일시 중단, 다시 시작, 확인 및 업데이트할 수 있습니다.
...
이 역할에 포함된 <strong>Set-MailboxDatabaseCopy</strong> 및 <strong>Remove-MailboxDatabaseCopy</strong> cmdlet을 사용하려면 구성 또는 제거할 데이터베이스가 데이터베이스 범위 내에 있고 데이터베이스가 상주하는 서버는 서버 범위 내에 있어야 합니다. 범위에 대한 자세한 내용은 [관리 역할 범위 이해 (영문)](understanding-management-role-scopes-exchange-2013-help.md)를 참조하십시오.
