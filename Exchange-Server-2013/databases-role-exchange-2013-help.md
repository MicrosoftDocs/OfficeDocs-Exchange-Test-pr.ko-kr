---
title: '데이터베이스 역할: Exchange 2013 Help'
TOCTitle: 데이터베이스 역할
ms:assetid: 317d9945-668b-448e-963f-f7127b9b3d79
ms:mtpsurl: https://technet.microsoft.com/ko-kr/library/Dd876868(v=EXCHG.150)
ms:contentKeyID: 50482818
ms.date: 05/22/2018
...
`Databases` 관리 역할을 사용하면 관리자가 개별 서버에서 사서함 데이터베이스를 만들고 관리, 탑재 및 분리할 수 있습니다.
...
이 역할에 포함된 <strong>Move-DatabasePath</strong> cmdlet을 사용하려면 구성하려는 데이터베이스가 데이터베이스 범위 내에 있어야 하고 해당 데이터베이스가 서버 범위 내의 서버에 있어야 합니다.

또한 이 역할에 포함된 <strong>Remove-MailboxDatabase</strong> cmdlet을 사용하려면 제거하려는 데이터베이스가 데이터베이스 범위 내에 있거나 해당 데이터베이스가 서버 범위 내의 서버에 있어야 합니다. 이는 데이터베이스 또는 서버 범위를 사용하여 사서함 데이터베이스를 제거할 수 있는 사용자를 제어함을 의미합니다.

자세한 내용은 [관리 역할 범위 이해 (영문)](understanding-management-role-scopes-exchange-2013-help.md) 항목을 참조하십시오.
