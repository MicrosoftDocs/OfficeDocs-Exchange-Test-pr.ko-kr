...
이 역할에 포함된 **Move-DatabasePath** cmdlet을 사용하려면 구성하려는 데이터베이스가 데이터베이스 범위 내에 있어야 하고 해당 데이터베이스가 서버 범위 내의 서버에 있어야 합니다.

또한 이 역할에 포함된 **Remove-MailboxDatabase** cmdlet을 사용하려면 제거하려는 데이터베이스가 데이터베이스 범위 내에 있거나 해당 데이터베이스가 서버 범위 내의 서버에 있어야 합니다. 이는 데이터베이스 또는 서버 범위를 사용하여 사서함 데이터베이스를 제거할 수 있는 사용자를 제어함을 의미합니다.
