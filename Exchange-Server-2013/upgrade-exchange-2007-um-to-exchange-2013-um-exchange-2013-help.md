...
2.  Exchange 관리 셸 프롬프트에 스크립트 경로를 입력합니다. 예를 들어, **cd "D:\\Program Files\\Microsoft\\Exchange Server\\V15\\Scripts"**를 입력하고 Enter 키를 누릅니다.

3.  셸 프롬프트에 **".\\MigrateUMCustomPrompts"**를 입력한 다음 Enter 키를 누릅니다.
...
1.  
    ...
    EAC에서 **통합 메시징** \> **UM 다이얼 플랜**으로 이동합니다. 자동 전화 교환을 추가할 UM 다이얼 플랜을 선택한 다음 **편집**![편집 아이콘](images/JJ218640.6f53ccb2-1f13-4c02-bea0-30690e6ea71d(EXCHG.150).gif "편집 아이콘")을 클릭합니다.
...
업그레이드 프로세스 동안 Exchange 2007 사서함 서버 및 Exchange 2013 사서함 서버 둘 다에서 일정 기간 동안 UM 사용 가능 사서함이 포함됩니다. 그러나 모든 UM 사용 가능 사용자를 Exchange 2013 사서함 서버로 이동하려면 Exchange 2013 서버의 셸에서 **New-MoveRequest** cmdlet을 사용하거나 EAC를 사용하여 모든 속성과 설정(사용자의 PIN 포함)을 보존해야 합니다.
...
1.  EAC에서 **받는 사람** \> **마이그레이션**을 클릭한 후 **추가**![편집 아이콘](images/JJ218640.6f53ccb2-1f13-4c02-bea0-30690e6ea71d(EXCHG.150).gif "편집 아이콘")를 클릭합니다.
...
3.  작업 창에서 다음 중 하나를 클릭합니다.
    ...
    1.  **즉시 사용 안 함** 옵션을 선택하면 통합 메시징 서버에 연결되어 있는 모든 통화의 연결이 끊어집니다.
    
    2.  **통화 완료 후 사용 안 함** 옵션을 선택하면 통합 메시징 서버가 새로운 통화를 받아들이지 않고 모든 통화가 처리된 후에야 사용할 수 없도록 설정됩니다.
...
5.  확인 대화 상자에서 **예**를 클릭하여 UM 다이얼 플랜에서 Exchange 2007 통합 메시징 서버 삭제를 확인합니다.
