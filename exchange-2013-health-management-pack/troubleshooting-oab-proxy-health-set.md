---
title: OAB 문제를 해결 합니다. 프록시 상태 설정
TOCTitle: OAB 문제를 해결 합니다. 프록시 상태 설정
ms:assetid: b717fc00-a787-44d6-8ccb-0eb4b2ea9e73
ms:mtpsurl: https://technet.microsoft.com/ko-kr/library/ms.exch.scom.oab.proxy(v=EXCHG.150)
ms:contentKeyID: 53275595
ms.date: 03/06/2017
mtps_version: v=EXCHG.150
ms.translationtype: MT
---

# OAB 문제를 해결 합니다. 프록시 상태 설정
...
OAB.Proxy의 상태가 비정상임을 지정하는 경고가 표시되면 사용자의 OAB 사용을 막을 수 있는 문제가 있는 것입니다.

## 설명

OAB 서비스는 다음 프로브 및 모니터를 사용하여 모니터링됩니다.
...
2.  메시지 세부 정보에서 경고의 정확한 원인에 대한 정보를 제공합니다. 대부분의 경우에는 메시지 세부 정보에서 근본 원인을 파악하는 데 충분한 문제 해결 정보를 제공합니다. 메시지 세부 정보가 명확하지 않은 경우 다음을 수행합니다.
    ...
    1.  Exchange 관리 셸를 열고 다음 명령을 실행하여 경고를 보낸 상태 설정의 세부 정보를 검색합니다.
        ...
        예를 들어 server1.contoso.com에 대한 OAB.Proxy 상태 설정 세부 정보를 검색하려면 다음 명령을 실행합니다.
    ...
    3.  비정상 상태인 모니터에 대해 관련 프로브를 다시 실행합니다. 관련 프로브를 찾으려면 Explanation 섹션의 표를 참조하십시오. 이렇게 하려면 다음 명령을 실행합니다.
        ...
        예를 들어 오류 발생 모니터가 **OABProxyTestMonitor**라고 가정하면 해당 모니터와 연결된 프로브는 **OABProxyTestProbe**입니다. server1.contoso.com에서 이 프로브를 실행하려면 다음 명령을 실행합니다.
...
1.  CAS에서 프로토콜 로그를 검토합니다. 프로토콜 로그는 CAS에서 *\<Exchange 서버 설치 디렉터리\>*\\Logging\\HttpProxy*\\\<프로토콜\>* 폴더에 있습니다.

2.  테스트 사용자 계정을 만든 다음 테스트 사용자 계정을 사용하여 CAS에 로그온합니다. 예를 들어 https:// *\<서버 이름\>*/owa를 사용하여 로그온합니다.

3.  IIS 관리자를 시작한 후 문제를 보고하는 서버에 연결하여 **MSExchangeOABAppPool** 응용 프로그램 풀이 CAS에서 실행되고 있는지 확인합니다.

4.  **응용 프로그램 풀**을 클릭한 후 셸에서 다음 명령을 실행하여 **MSExchangeOABAppPool** 응용 프로그램 풀을 재순환합니다.
...
[오프라인 주소록](https://technet.microsoft.com/ko-kr/library/bb232155\(v=exchg.150\))
